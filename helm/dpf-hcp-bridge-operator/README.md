...
- [Usage](#usage)
  ...
  - [Creating a DPFHCPBridge CR](#creating-a-dpfhcpbridge-cr)
  - [Monitoring DPFHCPBridge Resources](#monitoring-dpfhcpbridge-resources)
...
```bash
helm install dpf-hcp-bridge-operator oci://quay.io/lhadad/charts/dpf-hcp-bridge-operator \
  --version 0.1.0 \
  --namespace dpf-hcp-bridge-system --create-namespace
```
...
dpf-hcp-bridge-operator-xxxxxxxxxx-xxxxx       1/1     Running   0          30s
```
...
```bash
helm upgrade dpf-hcp-bridge-operator oci://quay.io/lhadad/charts/dpf-hcp-bridge-operator \
  --version 0.1.0 \
  --values values.yaml \
  --namespace dpf-hcp-bridge-system --create-namespace
```
...
Before creating a DPFHCPBridge CR, create the required secrets in the **same namespace** as your DPFHCPBridge CR:

```bash
# Create namespace for your DPFHCPBridge CR (if it doesn't exist)
kubectl create namespace my-dpu-clusters

# Create pull secret (must be in same namespace as DPFHCPBridge CR)
kubectl create secret generic my-pull-secret \
  --from-file=.dockerconfigjson=/path/to/pull-secret.json \
  --namespace my-dpu-clusters

# Create SSH key secret (must be in same namespace as DPFHCPBridge CR)
kubectl create secret generic my-ssh-key \
  --from-file=id_rsa.pub=/path/to/id_rsa.pub \
  --namespace my-dpu-clusters
```

### Creating a DPFHCPBridge CR

Once the operator is installed and secrets are created, you can create DPFHCPBridge CRs to provision DPU clusters.
...
kubectl describe dpfhcpbridge prod-dpu-cluster -n my-dpu-clusters

# Watch status changes
kubectl get dpfhcpbridge prod-dpu-cluster -n my-dpu-clusters -w
...
```bash
kubectl get dpfhcpbridge prod-dpu-cluster -n my-dpu-clusters -o jsonpath='{.status}' | jq
```
...
- `conditions`: Detailed condition information organized by category:
  - **DPFHCPBridge-specific conditions:**
    - `Ready`: Overall operational status of the DPFHCPBridge
  ...
  - **Validation conditions:**
    ...
    - `DPUClusterInUse`: DPUCluster is not already in use by another DPFHCPBridge
...
```bash
helm rollback dpf-hcp-bridge-operator --namespace dpf-hcp-bridge-system
```
...
```bash
helm uninstall dpf-hcp-bridge-operator --namespace dpf-hcp-bridge-system
```
...
**WARNING**: Deleting DPFHCPBridge CRs will trigger cleanup of associated HostedClusters and NodePools. Ensure you have backups before deleting production resources.
...
```bash
kubectl get pods -n dpf-hcp-bridge-system
kubectl describe pod -n dpf-hcp-bridge-system -l control-plane=controller-manager
...
kubectl get dpfhcpbridge <name> -n <namespace> -o jsonpath='{.status.conditions}' | jq
```
...
kubectl get secret -n <dpfhcpbridge-namespace> | grep kubeconfig
```
...
git clone https://github.com/rh-ecosystem-edge/dpf-hcp-bridge-operator
cd dpf-hcp-bridge-operator

# Install from local chart
helm install dpf-hcp-bridge-operator ./helm/dpf-hcp-bridge-operator \
  --namespace dpf-hcp-bridge-system --create-namespace

# Or with development values
helm install dpf-hcp-bridge-operator ./helm/dpf-hcp-bridge-operator \
  --values helm/dpf-hcp-bridge-operator/values-development.yaml \
  --namespace dpf-hcp-bridge-system --create-namespace
```
...
```bash
helm install dpf-hcp-bridge-operator ./helm/dpf-hcp-bridge-operator --dry-run --debug
```
...
# Production
helm install dpf-hcp-bridge-operator ./helm/dpf-hcp-bridge-operator \
  --values helm/dpf-hcp-bridge-operator/values-production.yaml
```
