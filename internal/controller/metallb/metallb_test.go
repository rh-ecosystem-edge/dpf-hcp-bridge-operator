...
var _ = Describe("MetalLB Manager", func() {
	...
	Describe("ConfigureMetalLB", func() {
		Context("when ShouldExposeThroughLoadBalancer is false", func() {
			It("should skip MetalLB configuration", func() {
				// Given: DPFHCPBridge with SingleReplica and no VirtualIP
				...
				result, err := manager.ConfigureMetalLB(ctx, bridge)
		...
		Context("when MetalLB resources do not exist", func() {
			It("should create IPAddressPool and L2Advertisement", func() {
				// Given: DPFHCPBridge with HighlyAvailable and VirtualIP
				bridge := &provisioningv1alpha1.DPFHCPBridge{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "test-bridge",
						Namespace: "test-ns",
				...
				Expect(pool.Spec.AllocateTo.Namespaces).To(Equal([]string{"clusters-test-bridge"}))
				Expect(*pool.Spec.AutoAssign).To(BeTrue())
				Expect(pool.Labels[common.LabelDPFHCPBridgeName]).To(Equal("test-bridge"))
				Expect(pool.Labels[common.LabelDPFHCPBridgeNamespace]).To(Equal("test-ns"))

				// L2Advertisement created
				advert := &metallbv1beta1.L2Advertisement{}
				err = fakeClient.Get(ctx, types.NamespacedName{
					Name:      "advertise-test-bridge",
					Namespace: common.OpenshiftOperatorsNamespace,
				}, advert)
				Expect(err).NotTo(HaveOccurred())
				Expect(advert.Spec.IPAddressPools).To(Equal([]string{"test-bridge"}))
				Expect(advert.Labels[common.LabelDPFHCPBridgeName]).To(Equal("test-bridge"))
				Expect(advert.Labels[common.LabelDPFHCPBridgeNamespace]).To(Equal("test-ns"))

				// Condition set to True
				cond := meta.FindStatusCondition(bridge.Status.Conditions, provisioningv1alpha1.MetalLBConfigured)
		...
		Context("when MetalLB resources exist with correct spec", func() {
			It("should not update resources", func() {
				// Given: DPFHCPBridge with existing MetalLB resources
				bridge := &provisioningv1alpha1.DPFHCPBridge{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "test-bridge",
						Namespace: "test-ns",
				...
				existingPool := &metallbv1beta1.IPAddressPool{
					ObjectMeta: metav1.ObjectMeta{
						...
						Labels: map[string]string{
							common.LabelDPFHCPBridgeName:      "test-bridge",
							common.LabelDPFHCPBridgeNamespace: "test-ns",
						},
					},
					Spec: metallbv1beta1.IPAddressPoolSpec{
						Addresses: []string{"192.168.1.100/32"},
						AllocateTo: &metallbv1beta1.ServiceAllocation{
							Namespaces: []string{"clusters-test-bridge"},
						},
				...
				existingAdvert := &metallbv1beta1.L2Advertisement{
					...
					Spec: metallbv1beta1.L2AdvertisementSpec{
						IPAddressPools: []string{"test-bridge"},
					},
				}

				fakeClient = fake.NewClientBuilder().
					WithScheme(scheme).
					WithObjects(bridge, existingPool, existingAdvert).
					WithStatusSubresource(bridge).
					Build()
		...
		Context("when MetalLB resources exist with incorrect spec (drift)", func() {
			It("should update IPAddressPool to correct drift", func() {
				// Given: DPFHCPBridge with drifted IPAddressPool
				bridge := &provisioningv1alpha1.DPFHCPBridge{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "test-bridge",
						Namespace: "test-ns",
				...
				fakeClient = fake.NewClientBuilder().
					WithScheme(scheme).
					WithObjects(bridge, driftedPool, existingAdvert).
					WithStatusSubresource(bridge).
					Build()
			...
			It("should update L2Advertisement to correct drift", func() {
				// Given: DPFHCPBridge with drifted L2Advertisement
				bridge := &provisioningv1alpha1.DPFHCPBridge{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "test-bridge",
						Namespace: "test-ns",
				...
				fakeClient = fake.NewClientBuilder().
					WithScheme(scheme).
					WithObjects(bridge, existingPool, driftedAdvert).
					WithStatusSubresource(bridge).
					Build()
	...
	Describe("Cleanup Handler", func() {
		Context("when both MetalLB resources exist", func() {
			It("should delete IPAddressPool and L2Advertisement", func() {
				...
				err := handler.Cleanup(ctx, bridge)
		...
		Context("when MetalLB resources don't exist", func() {
			It("should return nil (idempotent)", func() {
				// Given: DPFHCPBridge with no MetalLB resources
				bridge := &provisioningv1alpha1.DPFHCPBridge{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "test-bridge",
						Namespace: "test-ns",
		...
		Context("when only IPAddressPool exists", func() {
			It("should delete IPAddressPool and return success", func() {
				// Given: DPFHCPBridge with only IPAddressPool
				bridge := &provisioningv1alpha1.DPFHCPBridge{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "test-bridge",
						Namespace: "test-ns",
				...
				fakeClient = fake.NewClientBuilder().
					WithScheme(scheme).
					WithObjects(bridge, existingPool).
					Build()
		...
		Context("when only L2Advertisement exists", func() {
			It("should delete L2Advertisement and return success", func() {
				// Given: DPFHCPBridge with only L2Advertisement
				bridge := &provisioningv1alpha1.DPFHCPBridge{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "test-bridge",
						Namespace: "test-ns",
				...
				fakeClient = fake.NewClientBuilder().
					WithScheme(scheme).
					WithObjects(bridge, existingAdvert).
					Build()
		...
		Context("when cleanup is called multiple times (idempotency)", func() {
			It("should succeed on subsequent calls", func() {
				// Given: DPFHCPBridge with no resources
				bridge := &provisioningv1alpha1.DPFHCPBridge{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "test-bridge",
						Namespace: "test-ns",
	...
	Describe("Helper Functions", func() {
		Context("buildIPAddressPool", func() {
			It("should build correct IPAddressPool spec", func() {
				...
				manager = metallb.NewMetalLBManager(fakeClient, recorder)
				_, err := manager.ConfigureMetalLB(ctx, bridge)
				Expect(err).NotTo(HaveOccurred())
	...
	Describe("Edge Cases", func() {
		Context("when DPFHCPBridge has SingleReplica with VirtualIP", func() {
		...
		Context("when IPAddressPool exists without ownership labels (ownership conflict)", func() {
			It("should return error and not take ownership", func() {
				...
				fakeClient = fake.NewClientBuilder().
					WithScheme(scheme).
					WithObjects(bridge, unownedPool).
					WithStatusSubresource(bridge).
					Build()
				...
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(ContainSubstring("not owned by DPFHCPBridge"))
		...
		Context("when L2Advertisement exists without ownership labels (ownership conflict)", func() {
			It("should return error and not take ownership", func() {
				...
				fakeClient = fake.NewClientBuilder().
					WithScheme(scheme).
					WithObjects(bridge, ownedPool, unownedAdvert).
					WithStatusSubresource(bridge).
					Build()
	...
	Describe("Status Condition Management", func() {
		Context("when MetalLB configuration succeeds", func() {
			It("should set MetalLBConfigured condition to True with correct reason", func() {
				...
				Expect(cond.ObservedGeneration).To(Equal(bridge.Generation))
			})
		})
