...
func NewNodePoolManager(c client.Client) *NodePoolManager {
	return &NodePoolManager{Client: c}
}
...
func (nm *NodePoolManager) CreateNodePool(ctx context.Context, cr *provisioningv1alpha1.DPFHCPBridge) (ctrl.Result, error) {
	...
	if err == nil {
		// NodePool exists - verify ownership via labels
		if existingNP.Labels[LabelBridgeName] == cr.Name &&
			existingNP.Labels[LabelBridgeNamespace] == cr.Namespace {
			log.V(1).Info("NodePool already exists with matching labels",
				"nodePool", npName,
		...
		return ctrl.Result{}, fmt.Errorf("nodePool %s exists in %s but is owned by different DPFHCPBridge (labels mismatch)", npName, npNamespace)
	}
