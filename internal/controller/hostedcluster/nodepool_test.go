...
var _ = Describe("NodePool Builder", func() {
	...
	Context("Basic NodePool Fields", func() {
		It("should set correct metadata", func() {
			...
			Expect(np.Namespace).To(Equal(ClustersNamespace))
			Expect(np.Labels[LabelBridgeName]).To(Equal("test-bridge"))
			Expect(np.Labels[LabelBridgeNamespace]).To(Equal("default"))
		})
