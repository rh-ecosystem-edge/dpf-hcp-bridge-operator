...
var _ = Describe("HostedCluster Builder", func() {
	...
	Context("Basic HostedCluster Fields", func() {
		It("should set correct metadata", func() {
			...
			Expect(hc.Namespace).To(Equal(ClustersNamespace))
			Expect(hc.Labels[LabelBridgeName]).To(Equal("test-bridge"))
			Expect(hc.Labels[LabelBridgeNamespace]).To(Equal("default"))
		})
